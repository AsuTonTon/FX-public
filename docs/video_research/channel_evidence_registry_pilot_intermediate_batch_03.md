...
`channel_evidence_registry_pilot_intermediate_batch_03_schema_usable_with_small_adjustments`
...
- A full future registry should probably add a lightweight `source_granularity` field, such as `per_video_summary`, `aggregate_inventory`, `batch_synthesis`, or `audit_status`, because the distinction matters during review.
- Very wide tables are hard to review in Markdown. A future larger registry may need one compact overview table plus per-row detail sections, while still avoiding CSV/JSON dumps unless explicitly authorized.

## 9. Recommended next docs-only task

Recommended next task:

`[codex] Review the Intermediate Batch 03 pilot evidence registry and decide whether to adopt source_granularity before any larger backfill`

Reason:

- the pilot suggests the core schema works;
- a small schema refinement may reduce ambiguity before more rows are added;
- the next task can remain docs-only and should not perform a full registry backfill yet.
